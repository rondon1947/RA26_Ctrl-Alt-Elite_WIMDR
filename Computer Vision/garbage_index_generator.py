...
def get_pop_density(x, y):
    loc = str(x)+','+str(y)
    res = requests.post('https://maps.googleapis.com/maps/api/geocode/json?latlng='+loc+'&key=AIzaSyDAaGzlR9XHDGvdBHn2ZpHl7RU_tWMgil0')
    res = (res.json())
    try:
        temp = res['results'][3]
...
def GetUnAssignedIndexes():
    ...
    for i in range(len(final_latitude)):
        final_index.append( (locations[i]*0.35) + (final_animals[i]*0.15) + (pop_den[i]*0.25) + (water_body[i]*0.25))
